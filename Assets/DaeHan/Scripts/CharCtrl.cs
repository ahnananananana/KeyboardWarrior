...
public class CharCtrl : Character
{
    ...
    {
        GameObject obj = Instantiate(SwordAtkBox) as GameObject;
        handatk = obj.GetComponent<HandAtk>();
        obj.transform.position = SwordAtkBoxTR.transform.position;
        obj.transform.rotation = SwordAtkBoxTR.transform.rotation;
        handatk.character = GetComponent<Player>();
    }
    ...
    protected void Moving() // 이동함수
    {
        ...
        float delta = player.m_MoveSpeed.m_CurrentValue * Time.smoothDeltaTime;
